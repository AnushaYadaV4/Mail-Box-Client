    ...
    if (isLogin) {
      if (enteredMail.trim().length === 0 || enteredPass.trim().length === 0) {
        ...
        sendRequest(
          {
            request: "post",
            url: "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAu6zYqqYTh_UX2bcfSu1IhAnFamv4M7qU",
            data: authObj,
      ...
      if (
        ...
        sendRequest(
          {
            request: "post",
            url: "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAu6zYqqYTh_UX2bcfSu1IhAnFamv4M7qU",
            data: authObj,
            header: { "Content-Type": " application/json" },
          },
          resData
